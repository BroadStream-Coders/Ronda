...
## [RM-115] Botón de agregar fila visible y con espaciado parejo (2026-09-06 23:40)
`AddRowButton` era un `ghost` sin borde ni fondo: sobre el `bg-card` de la columna no se distinguía hasta pasarle el mouse, en los dos temas. Ahora lleva borde punteado y `bg-muted/30`, el mismo lenguaje del botón de agregar columna. El espacio de arriba lo ponía `RowsContainer` con `py-3`, o sea **dentro** del área que hace scroll: con la lista desplazada el botón quedaba pegado al contenido cortado y al final del scroll el hueco era el doble. El contenedor pasó a `pt-3` y el botón trae su propio `py-3`. Alcanza a los 12 colectores con listas.

## [RM-113] Cronos: fila compacta, cinco eventos fijos y carga rápida de imágenes (2026-09-06 19:46)
La fila de evento pasó de vertical a la tarjeta de Álbum —número dentro del borde, fecha y título al centro, miniatura de 5rem a la derecha—: de ~320px de alto a ~96px, así que las cinco entran sin scroll. Antes el `ImagePicker` con `crop` tomaba el ancho completo de la fila y los `Input` iban con `border-transparent bg-background`, que en modo claro se veía blanco sobre blanco; ahora usan el `Input` del sistema sin overrides de color. Cada ronda quedó fija en **5 eventos** (lo que pinta el juego, `SLOT_COUNT`): sin botón de agregar, sin tacho, sin contador, y al cargar un zip cada ronda se normaliza a cinco avisando si el archivo traía más. La validación ahora etiqueta por `Ronda N` en vez de por el título escrito, y el vocabulario de la UI quedó todo en "ronda" (botón y avisos incluidos); las claves del JSON siguen siendo `groups`. Cierra también [[WL-014]] con la carga rápida de imágenes.
...
## [RM-112] Seis sobres fijos y carga rápida de imágenes en Álbum (2026-09-04 14:59)
El colector de Álbum pasó a tener **exactamente 6 sobres**, ni uno más ni uno menos: arrancan vacíos, no se agregan ni se eliminan, y un zip con más de 6 carga los primeros y avisa. Cada sobre queda fijo en 5 cartas y perdió el contador `5/5` del header, que ya no informaba nada. El validador ahora exige el **título** del sobre (antes solo pedía pregunta e imagen), así que una sesión válida es 6 títulos + 30 preguntas + 30 fotos, el mínimo para que el juego sea usable. Del kit salió **`QuickImages`**, hermano de `QuickLoad`: sube varias imágenes de golpe y las reparte en orden desde la primera carta; en Álbum va a la izquierda del botón de planilla, con las etiquetas cortas "Imágenes" y "Planilla" porque los textos completos no entran en 320px. `QuickLoad` ganó dos props opcionales (`label` y `leading`) y `GroupsContainer`/`GroupColumn` hicieron opcionales sus botones de agregar y eliminar; los otros 15 colectores no cambian. También se intercambiaron los íconos con Galería de Fotos. Con esto cierra [[TD-113]]; lo que el arreglo **no** resuelve queda en [[TD-116]]: el tope está escrito a mano en Álbum y no hay forma general de declarar el límite de un juego ni de mostrárselo al productor.
